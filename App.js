...
const FREE_LIMIT = 3;
const getRunsUsed = () => { try { return parseInt(localStorage.getItem("ek_runs") || "0"); } catch { return 0; } };
const setRunsUsed = (n) => { try { localStorage.setItem("ek_runs", String(n)); } catch {} };
const getSavedKey = () => { try { return localStorage.getItem("ek_key") || ""; } catch { return ""; } };
const saveKey = (k) => { try { localStorage.setItem("ek_key", k); } catch {} };
const getSavedProvider = () => { try { return localStorage.getItem("ek_provider") || "anthropic"; } catch { return "anthropic"; } };
const saveProvider = (p) => { try { localStorage.setItem("ek_provider", p); } catch {} };
...
Respond ONLY in JSON:
{
  ...
  "expected_accuracy": 9
      ...
      const fixed = cleaned.replace(/(?<=":[\s]*")([\s\S]*?)(?="[\s]*[,}])/g, m => m.replace(/\n/g, "\\n").replace(/\r/g, "\\r").replace(/\t/g, "\\t"));
      let parsed;
